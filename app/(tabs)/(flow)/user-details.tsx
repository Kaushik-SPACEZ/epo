      ...
      if (user?.id) {
        try {
          ...
          if (response.success && response.data) {
            ...
            // Update the initial set with fetched data
            const updatedC: CData = {
              name: userData.name || '',
            ...
            const updatedD: DData = {
              bizName: userData.company_name || '',
              contact: userData.name || '',
              email: userData.email || '',
              phone: userData.phone || '',
              udyam: userData.udyam_number || '',
              address: userData.address || '',
              city: userData.city || '',
              pincode: userData.pincode || '',
          ...
          console.error('[UserDetails] Failed to fetch user data:', error);
        }
      }
    };
  ...
  const handleSave = async () => {
    ...
    if (userType === 'customer') {
      ...
      if (!c.name.trim()) {
        ...
        errs.cName = 'Name must be at least 2 characters';
      } else if (!/^[a-zA-Z\s]+$/.test(c.name)) {
        errs.cName = 'Name should only contain letters and spaces';
      }
      
      // Email validation
      if (!c.email.trim()) {
        errs.cEmail = 'Email is required';
      } else if (!validateEmail(c.email)) {
        errs.cEmail = 'Enter a valid email address (e.g. name@example.com)';
      }
      
      // Phone validation
      if (!c.phone.trim()) {
        errs.cPhone = 'Phone number is required';
      } else if (!validatePhone(c.phone)) {
        errs.cPhone = 'Phone number must be exactly 10 digits';
      } else if (!/^[6-9]/.test(c.phone.trim())) {
        errs.cPhone = 'Phone number must start with 6, 7, 8, or 9';
      }
      ...
      if (!c.address.trim()) {
        ...
        errs.cAddress = 'Address must be at least 10 characters';
      }
      
      // City validation
      if (!c.city.trim()) {
        errs.cCity = 'City is required';
      } else if (c.city.trim().length < 2) {
        errs.cCity = 'City name must be at least 2 characters';
      } else if (!/^[a-zA-Z\s]+$/.test(c.city)) {
        errs.cCity = 'City name should only contain letters and spaces';
      }
      
      // Pincode validation
      if (!c.pincode.trim()) {
        errs.cPincode = 'Pincode is required';
      } else if (!validatePincode(c.pincode)) {
        errs.cPincode = 'Pincode must be exactly 6 digits';
      }
    } else {
      // Business name validation
      if (!d.bizName.trim()) {
        errs.dBizName = 'Business name is required';
      } else if (d.bizName.trim().length < 3) {
        errs.dBizName = 'Business name must be at least 3 characters';
      }
      
      // Contact person validation
      if (!d.contact.trim()) {
        errs.dContact = 'Contact person name is required';
      } else if (d.contact.trim().length < 2) {
        errs.dContact = 'Contact person name must be at least 2 characters';
      } else if (!/^[a-zA-Z\s]+$/.test(d.contact)) {
        errs.dContact = 'Contact person name should only contain letters and spaces';
      }
      
      // Email validation
      if (!d.email.trim()) {
        errs.dEmail = 'Email is required';
      } else if (!validateEmail(d.email)) {
        errs.dEmail = 'Enter a valid email address (e.g. name@example.com)';
      }
      
      // Phone validation
      if (!d.phone.trim()) {
        errs.dPhone = 'Phone number is required';
      } else if (!validatePhone(d.phone)) {
        errs.dPhone = 'Phone number must be exactly 10 digits';
      } else if (!/^[6-9]/.test(d.phone.trim())) {
        errs.dPhone = 'Phone number must start with 6, 7, 8, or 9';
      }
      
      // UDYAM validation
      if (!d.udyam.trim()) {
        errs.dUdyam = 'UDYAM number is required';
      } else if (d.udyam.trim().length !== 12) {
        errs.dUdyam = 'UDYAM number must be exactly 12 characters';
      } else if (!/^[A-Z0-9]+$/.test(d.udyam.trim())) {
        errs.dUdyam = 'UDYAM number should only contain uppercase letters and numbers';
      }
      
      // Address validation
      if (!d.address.trim()) {
        errs.dAddress = 'Address is required';
      } else if (d.address.trim().length < 10) {
        errs.dAddress = 'Address must be at least 10 characters';
      }
      
      // City validation
      if (!d.city.trim()) {
        errs.dCity = 'City is required';
      } else if (d.city.trim().length < 2) {
        errs.dCity = 'City name must be at least 2 characters';
      } else if (!/^[a-zA-Z\s]+$/.test(d.city)) {
        errs.dCity = 'City name should only contain letters and spaces';
      }
      
      // Pincode validation
      if (!d.pincode.trim()) {
        errs.dPincode = 'Pincode is required';
      } else if (!validatePincode(d.pincode)) {
        errs.dPincode = 'Pincode must be exactly 6 digits';
      }
      
      // GST validation (optional field)
      if (d.gst && d.gst.trim()) {
        if (d.gst.trim().length !== 15) {
          errs.dGst = 'GST number must be exactly 15 characters';
        } else if (!/^[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z]{1}[1-9A-Z]{1}Z[0-9A-Z]{1}$/.test(d.gst.trim())) {
          errs.dGst = 'Invalid GST number format';
        }
      }
    }
    ...
    if (user?.id) {
      try {
        ...
        const updateData = userType === 'customer' ? {
          ...
          email: d.email,
          phone: d.phone,
          company_name: d.bizName,
          address: d.address,
          city: d.city,
          state: 'Tamil Nadu',
          pincode: d.pincode,
          gst_number: d.gst || undefined,
          udyam_number: d.udyam,
        };

        const response = await api.users.update(userId, updateData);
        
        if (!response.success) {
          showAlert('Update Failed', response.error || 'Failed to update profile');
          return;
        }
      } catch (error: any) {
        console.error('[UserDetails] Update error:', error);
        showAlert('Error', error.response?.data?.error || 'Failed to update profile');
        return;
      }
    }
        ...
        >
          ...
          <View style={[styles.formCard, isEditing && styles.formCardEditing]}>
            ...
            <View style={styles.formHeader}>
              ...
              {/* Change button - Hidden for now, will be enabled in future */}
              {/* {!isEditing && (
