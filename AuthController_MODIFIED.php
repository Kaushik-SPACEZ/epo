...
{
    // ─── POST /auth/register ─────────────────────────────────────────────────
    // MODIFIED: Now sets approval_status='pending' and is_active=false
    public function register(Request $request): void
    {
        ...
        try {
            // MODIFIED: Create user with pending approval status
        ...
        // MODIFIED: Return success without tokens (user cannot login yet)
        Response::success(
            [
                'user_id' => $userId,
    ...
    // ─── POST /auth/login ────────────────────────────────────────────────────
    // MODIFIED: Now checks approval_status before allowing login
    public function login(Request $request): void
    {
        try {
            ...
            // NEW: Check approval status BEFORE checking is_active
    ...
    public function refresh(Request $request): void
    {
        try {
            ...
            [$access, $refresh, $refreshExpiry] = $this->issueTokenPair((int) $stored['user_id'], $request);
    ...
    /**
     * Send approval email to user (called by AdminUserController)
     */
    ...
    /**
     * Send rejection email to user (called by AdminUserController)
     */
